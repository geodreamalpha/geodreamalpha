...
namespace CombatSystemComponent
{
    ...
    class EnemyGenerator
    {
        ...
        public void CreateNewEnemies(CombatSystemAssets assets, Transform player)
        {
            ...
            PlayerBehavior playerBehavior = player.root.gameObject.GetComponent<PlayerBehavior>();

            if (playerBehavior.isFirebaseStatsLoaded)
                while (currentEnemies < MAX_ENEMIES && enemyInfo.Count > 0)
                {
                    ...
                    HelperBase enemyBase = enemies[enemies.Count - 1].GetComponent<HelperBase>();
                    enemyBase.SetAssets(assets);
                    enemyBase.levelStats = playerBehavior.levelStats;
                    enemyBase.AdjustInGameStats();
                    enemyBase.AdjustHealthAndStamina();
                }
        }
